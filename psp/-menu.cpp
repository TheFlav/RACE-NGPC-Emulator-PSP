...
static int  psp_save_options_game();
static void psp_load_options_game();
...
PL_MENU_OPTIONS_BEGIN(FrameskipOptions)
  PL_MENU_OPTION("30fps", 1)
  PL_MENU_OPTION("60fps", 0)
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(VsyncOptions)
  PL_MENU_OPTION("Normal VSync", 0)
  PL_MENU_OPTION("Perfect VSync", 1)
  PL_MENU_OPTION("Intelligent VSync", 2)
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(BorderOptions)
  PL_MENU_OPTION("No border", 0)
  PL_MENU_OPTION("Default", 1)
  PL_MENU_OPTION("Custom", 2)
  PL_MENU_OPTION("Baseball",3)
  PL_MENU_OPTION("Casino",4)
  PL_MENU_OPTION("Pachinko",5)
  PL_MENU_OPTION("Dating sim",6)
  PL_MENU_OPTION("Board",7)  
  PL_MENU_OPTION("Generic RPG",8)  
PL_MENU_OPTIONS_END
    ...
    do
    {
      ...
      case TAB_OPTIONS:
       ...
       // pl_menu_select_option_by_value(item, (void*)(int)UiMetric.Animate);
        ...
        if (!GAME_LOADED){psp_save_options();}
        else{
        psp_save_options_game();}
        break;
      case TAB_STATE:
...
static void psp_load_border_name()
{
   ...
   if (psp_options.border >= 3){
   sprintf(BorderPath, "%sborders/", pl_psp_get_app_directory());
  
   const char *border_name = pl_file_get_filename(CURRENT_GAME);
   pl_file_path path;
   if (psp_options.border == 3){
   snprintf(path, sizeof(path) - 1, "%simg1.png", BorderPath, border_name);}
   if (psp_options.border == 4){
   snprintf(path, sizeof(path) - 1, "%simg2.png", BorderPath, border_name);}   
   if (psp_options.border == 5){
   snprintf(path, sizeof(path) - 1, "%simg3.png", BorderPath, border_name);}
   if (psp_options.border == 6){
   snprintf(path, sizeof(path) - 1, "%simg4.png", BorderPath, border_name);}   
   if (psp_options.border == 7){
   snprintf(path, sizeof(path) - 1, "%simg5.png", BorderPath, border_name);}   
   if (psp_options.border == 8){
   snprintf(path, sizeof(path) - 1, "%simg6.png", BorderPath, border_name);}         
   bordername = path;}
...
static void psp_load_options_game()
{
  ...
  const char *config_name = pl_file_get_filename(CURRENT_GAME);
  pl_file_path path;
  snprintf(path, sizeof(path) - 1, "%s%soptions.ini",
  IniPath, config_name);
  ...
  psp_options.frame_skip = pl_ini_get_int(&file, "Video", "Frame Skipping", fskip);
  ...
  UiMetric.Animate = pl_ini_get_int(&file, "Menu", "Animate", 0);
...
static void psp_load_options()
{
  ...
  psp_options.pos_vert = pl_ini_get_int(&file, "Video", "PosVert", 22);
  UiMetric.Animate = pl_ini_get_int(&file, "Menu", "Animate", 0);
  psp_options.border = pl_ini_get_int(&file, "Enhancements", "Border", 0);
  psp_options.autofire = pl_ini_get_int(&file, "Input", "Autofire", 2);
  if (psp_options.autofire < 2)
    psp_options.autofire = 2;
