...
PL_MENU_OPTIONS_BEGIN(ScreenSizeOptions)
  ...
  PL_MENU_OPTION("Simple 2x - SW", SIMPLE_2X)
  PL_MENU_OPTION("Simple 2x Wide - SW", SIMPLE_W)  
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(FrameskipOptions)
  PL_MENU_OPTION("Auto", 2)
  PL_MENU_OPTION("30 Hz", 1)
  PL_MENU_OPTION("60 Hz", 0)
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(VsyncOptions)
  PL_MENU_OPTION("Normal", 0)
  PL_MENU_OPTION("Precise", 1)
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(BorderOptions)
  PL_MENU_OPTION("No border", 0)
  PL_MENU_OPTION("PNG Border", 1) 
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(ScreenPosOptions)
  PL_MENU_OPTION("2", 2)
  PL_MENU_OPTION("8", 8)
  PL_MENU_OPTION("16", 16)
  PL_MENU_OPTION("18", 18)  
  PL_MENU_OPTION("Auto", 0)   
  PL_MENU_OPTION("22", 22)    
  PL_MENU_OPTION("24", 24)   
  PL_MENU_OPTION("32", 32)     
PL_MENU_OPTIONS_END
...
PL_MENU_OPTIONS_END
PL_MENU_OPTIONS_BEGIN(LangOptions)
  PL_MENU_OPTION("English", 0)
  PL_MENU_OPTION("Japanese", 1)
PL_MENU_OPTIONS_END
...
PL_MENU_ITEMS_BEGIN(OptionMenuDef)
  ...
  PL_MENU_ITEM("Screen position",OPTION_POSVX,ScreenPosOptions,"\026\244\020 Change screen position")
  PL_MENU_ITEM("VSYNC",PERFECT_SYNC,VsyncOptions,"\026\250\020 Enable/disable vsync options")
  ...
  PL_MENU_ITEM("Screen Refresh", OPTION_FRAME_SKIP,FrameskipOptions,"\026\250\020 Select number of frames to skip per update")
  ...
  PL_MENU_ITEM("Border",OPTION_BORDER,BorderOptions,"\026\250\020 Enable/disable PNG borders")
  PL_MENU_ITEM("Language",OPTION_LANG,LangOptions,"Language")  
...
PL_MENU_ITEMS_BEGIN(SystemMenuDef)
  ...
  PL_MENU_ITEM("Exit Emulator",SYSTEM_EXIT,NULL,"\026\001\020 Exit Emulator")
PL_MENU_ITEMS_END
  ...
  do
  {
    ...
    do
    {
      ...
      case TAB_OPTIONS:
        ...
        item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, PERFECT_SYNC);
        pl_menu_select_option_by_value(item, (void*)(int)psp_options.pvsync);        
        ...
        item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, OPTION_POSVX);
        pl_menu_select_option_by_value(item, (void*)(int)psp_options.pos_vert);        
       // item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, OPTION_ANIMATE);
       // pl_menu_select_option_by_value(item, (void*)(int)psp_options.language);
        ...
        pl_menu_select_option_by_value(item, (void*)(int)psp_options.border);
         item = pl_menu_find_item_by_id(&OptionUiMenu.Menu, OPTION_LANG);
        pl_menu_select_option_by_value(item, (void*)(int)psp_options.language);
        
               
        pspUiOpenMenu(&OptionUiMenu, NULL);
        
        if (pspUiConfirm("Save config?"))
         {
        if (juegon==1){
        psp_options.OF = 1;
        CreateOptions();}
    ...
    if (!ExitPSP)
    {
      ...
      if (ResumeEmulation)
      {
       ...
       /*
       0x05 Cotton - Fantastic Night Dreams - 60fps
       0x83 Bikkuriman 2000 - 60fps
       0x85 Densetsu no Ogre Battle - 60fps
       0x62 Mizuki Shigeru Youkai Shashin Kan - 60fps
       0x96 Nige-ron-pa - 60fps
       0x81 Oekaki - 60fps
       0x55 Pac-Man - 60fps
       0x32 Party Mail - 60fps
       0x06 Picture Puzzle - 60fps
       0x41 Puyo Pop - 60fps
       0x78 Soreike! Hanafuda Doujyou - 60fps
       0x94 Rockman Battle & Fighters - 60fps
       0x65 Gals Fighter - 60fps
       0x93 Gals Fighter EU - 60fps
       0x11 Fatal Fury - 60fps
       
       0x94 Rockman Battle & Fighters - 24v
       0x20 Puzzle Bobble Mini - 16v
       0x11 Super Real Mahjong - 16v
       0x31 Bust-A-Move Pocket EU - 16v
       0x90 Faselei! - 18v
       0x01 King of Fighters R-1 - 24v
       0x23 King of Fighters R-2 - 24v
       0x21 Metal Slug - 1st Mission - 16v
       0x63 Mezase! Kanji Ou - 8v
       0x30 Samurai Shodown 2 - 16v
       0x08 Samurai Shodown - 16v
       0x41 Puyo Pop - 16v
       0x69 SVC - The Match of The Millennium MONO - 0v    
       */
       
        // 0x11 Super Real Mahjong sound fix
        fixsoundmahjong=0;
        
        gfx_hacks=0;

       
        if (psp_options.pos_vert==0) {
        
        if (mainrom[0x000020] == 0x94 || mainrom[0x000020] == 0x01 || mainrom[0x000020] == 0x23)
        psp_options.pos_vert=24;
        
        if (mainrom[0x000020] == 0x05)
        psp_options.pos_vert=30;
                 
        if (mainrom[0x000020] == 0x90)
        psp_options.pos_vert=18;
        
        if (mainrom[0x000020] == 0x63)
        psp_options.pos_vert=8;  
        
        if (mainrom[0x000020] == 0x31 || mainrom[0x000020] == 0x08 || mainrom[0x000020] == 0x21 || mainrom[0x000020] == 0x30 || mainrom[0x000020] == 0x20 || mainrom[0x000020] == 0x41)
        psp_options.pos_vert=16;
        
        if (mainrom[0x000020] == 0x11 && mainrom[0x000021] == 0x01)
        psp_options.pos_vert=16;  
        
        if (tipo_consola==1 && mainrom[0x000020] == 0x69)
        psp_options.pos_vert=0;

        else 
        psp_options.pos_vert=22;         
        ...
        if (psp_options.frame_skip==2) {
  
        if ((mainrom[0x000020] == 0x11 && mainrom[0x000021] == 0x00 ) || mainrom[0x000020] == 0x93 || mainrom[0x000020] == 0x65 || mainrom[0x000020] == 0x05 || mainrom[0x000020] == 0x83 || mainrom[0x000020] == 0x85 || mainrom[0x000020] == 0x62 || mainrom[0x000020] == 0x96 || mainrom[0x000020] == 0x81 || mainrom[0x000020] == 0x55 || mainrom[0x000020] == 0x32 || mainrom[0x000020] == 0x06 || mainrom[0x000020] == 0x41 || mainrom[0x000020] == 0x78 || mainrom[0x000020] == 0x94 )
          psp_options.frame_skip=0;                               
         
         else
          psp_options.frame_skip=1;         }
          
          
        //NOTA HAcks
        // 0x07 Oelsol
          
        if (mainrom[0x000020] == 0x07  || mainrom[0x000020] == 0x30  || mainrom[0x000020] == 0x08  || mainrom[0x000020] == 0x10 || mainrom[0x000020] == 0x12  || (mainrom[0x000020] == 0x11 && mainrom[0x000021] == 0x01))
         gfx_hacks=1;          
                         ...
                         const void *path)
{
  ...
  //pspUiAlert("Resumen despues de cargar");

  system_sound_chipreset(); /* Reset sound */
  pl_rewind_reset(&Rewinder);

  
  idioma = psp_options.language;
...
static int OnSaveStateOk(const void *gallery, const void *item)
{
  ...
  path_save = (char*)malloc(strlen(SaveStatePath) + strlen(config_name) + 8);
  sprintf(path_save, "%s%s_%02i.rcs", SaveStatePath, config_name,
    ((const pl_menu_item*)item)->id);

  if (pl_file_exists(path_save) && pspUiConfirm("Load state?"))
  {
    if (psp_load_state(path_save))
    {
      /*NOTA*/
                           ...
                           const void *null)
{
  ...
  { 
    ...
    " ",
    "2014 TheElf",
    "\026http://theelf-megadev.com/psp",
    " ",
...
static int OnMenuOk(const void *uimenu, const void* sel_item)
{
  ...
  else
  {
    ...
    case SYSTEM_RESET:
     ...
     case SYSTEM_EXIT:
      if (pspUiConfirm("Quit Emulation?"))
      {
        /* Write flash data for current game */
        if (GAME_LOADED)
          writeSaveGameFile();
                             ...
                             const pl_menu_option* option)
{
  ...
  else
  {
    ...
    case OPTION_CONTROL_MODE:
      ...
      break;
    case PERFECT_SYNC:
      psp_options.pvsync = (int)option->value;
      break;
    case OPTION_POSVX:
      psp_options.pos_vert = (int)option->value;
      break;
    case OPTION_ANIMATE:
      psp_options.language = (int)option->value;
      break;
    case OPTION_AUTOFIRE:
      psp_options.autofire = (int)option->value;
      break;
    case OPTION_BORDER:
      psp_options.border = (int)option->value;
      break;
    case OPTION_LANG:
      psp_options.language = (int)option->value;
      break;      
 ...
 static void loadborder()
{
   ...
   if (psp_options.border==1){ 
   const char *border_name = pl_file_get_filename(CURRENT_GAME);
   snprintf(path, sizeof(path) - 1, "%s%sborder.png", BorderPath, border_name);
   bordername = path;
   customborder=1;
   ...
   snprintf(path, sizeof(path) - 1, "%sborder.png", BorderPath);
   customborder=0;}
...
static void LoadOptions_game()
{
  ...
  psp_options.display_mode = pl_ini_get_int(&file, "Video", "Display Mode", dmode);
  psp_options.frame_skip = pl_ini_get_int(&file, "Video", "Screen Refresh", fskip);
  psp_options.clock_freq = pl_ini_get_int(&file, "Video", "PSP Clock Frequency", cclock);
  ...
  psp_options.rewind_save_rate = pl_ini_get_int(&file, "Enhancements", "Rewind Save Rate", rrewind);
   pl_ini_get_string(&file, "File", "Game Path", NULL, 
                    GamePath, sizeof(GamePath));
  psp_options.pvsync = pl_ini_get_int(&file, "Video", "Vsync", vvsync);
  psp_options.pos_vert = pl_ini_get_int(&file, "Video", "PosVert", ppos);
  psp_options.language = pl_ini_get_int(&file, "Menu", "Language", 1);
  psp_options.OF = pl_ini_get_int(&file, "System", "OFile", 0); 
  psp_options.border = pl_ini_get_int(&file, "Enhancements", "Border", bbord);
  psp_options.autofire = pl_ini_get_int(&file, "Input", "Autofire", 2);
  if (psp_options.autofire < 2)
    psp_options.autofire = 2;

   int control_mode = pl_ini_get_int(&file, "Menu", "Control Mode", ccont);
...
static void LoadOptions()
{
  ...
  psp_options.display_mode = pl_ini_get_int(&file, "Video", "Display Mode", SIMPLE_2X);
  psp_options.frame_skip = pl_ini_get_int(&file, "Video", "Screen Refresh", 2);
  ...
  pl_ini_get_string(&file, "File", "Game Path", NULL, 
                    GamePath, sizeof(GamePath));
  psp_options.pvsync = pl_ini_get_int(&file, "Video", "Vsync", 1);
  psp_options.pos_vert = pl_ini_get_int(&file, "Video", "PosVert", 0);
  psp_options.language = pl_ini_get_int(&file, "Menu", "Language", 0);
  psp_options.OF = pl_ini_get_int(&file, "System", "OFile", 0); 
  psp_options.border = pl_ini_get_int(&file, "Enhancements", "Border", 1);
  psp_options.autofire = pl_ini_get_int(&file, "Input", "Autofire", 2);
  if (psp_options.autofire < 2)
    psp_options.autofire = 2;

  int control_mode = pl_ini_get_int(&file, "Menu", "Control Mode", 1);
  UiMetric.OkButton = (!control_mode)
                      ? PSP_CTRL_CROSS : PSP_CTRL_CIRCLE;
  UiMetric.CancelButton = (!control_mode)
                          ? PSP_CTRL_CIRCLE : PSP_CTRL_CROSS;
                          
  dmode = psp_options.display_mode;
  fskip = psp_options.frame_skip;
  cclock = psp_options.clock_freq;
  rrewind = psp_options.rewind_save_rate;
  vvsync = psp_options.pvsync;
  ppos = psp_options.pos_vert;
  ccont = control_mode;                        
  bbord = psp_options.border;                       
...
static int SaveOptions()
{
  ...
  pl_ini_set_int(&file, "Video", "Screen Refresh", 
  ...
  pl_ini_set_int(&file, "Video", "Vsync",
                 psp_options.pvsync);   
  pl_ini_set_int(&file, "Video", "PosVert", 
                  psp_options.pos_vert);                               
  pl_ini_set_int(&file, "Menu", "Language",
                 psp_options.language);
  pl_ini_set_int(&file, "Enhancements", "Border",
                psp_options.border);
...
static int CreateOptions()
{
  ...
  pl_ini_set_int(&file, "Video", "PosVert", 
                  psp_options.pos_vert);
  pl_ini_set_int(&file, "System", "OFile", 
                  psp_options.OF);                                                  
  pl_ini_set_int(&file, "Menu", "Language",
                psp_options.language);             
  pl_ini_set_int(&file, "Enhancements", "Border",
