 ...
 /*NOTA*/
  ...
  while (!ExitPSP)
  {
    ...
    while (!ExitPSP)
    {
      ...
      /* NOTA: Barra de seleccion rom Render selection box */
...
void pspUiOpenMenu(PspUiMenu *uimenu, const char *title)
{
  ...
  while (!ExitPSP)
  {
    ...
    /* NOTA: Tamanio otras barras Draw the highlight for selected item */
    if (!option_mode)
